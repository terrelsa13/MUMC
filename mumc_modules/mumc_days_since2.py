...
def get_days_since_played2(date_last_played,the_dict):
...
def get_days_since_created2(date_last_created,the_dict):
    return(get_days_since_played2(date_last_created,the_dict).replace('Played', 'Created', 1))
...
def convert_timeToString2(byUserId_item):
    for userId in byUserId_item:
        #if (not((userId == 'dynamic_behavior') or (userId == 'action_control') or (userId == 'action_type') or (userId == 'user_conditional') or
                #(userId == 'played_conditional') or (userId == 'action'))):
        if (not((userId == 'action') or (userId == 'user_conditional') or (userId == 'played_conditional') or (userId == 'action_control') or
                (userId == 'dynamic_behavior') or (userId == 'action_type'))):
