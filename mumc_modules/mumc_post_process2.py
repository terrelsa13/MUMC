...
from mumc_modules.mumc_console_info2 import print_post_processing_started2,print_post_processing_verbal_progress2,print_post_processing_verbal_progress_min_episode2,print_post_processing_completed2
from mumc_modules.mumc_days_since2 import convert_timeToString2
...
def build_behaviorPattern2(isMeeting_dict,behavior_pattern_dict,isbehavior_extraInfo_byUserId_Media):
...
def addItem_removeItem_fromDeleteList_usingBehavioralPatterns2(behavior_str,postproc_dict):
    ...
    if (('user_conditional' in isbehavior_extraInfo_byUserId_Media) and ('played_conditional' in isbehavior_extraInfo_byUserId_Media)):
        configuredBehavior=isbehavior_extraInfo_byUserId_Media['action']
        actionControl=isbehavior_extraInfo_byUserId_Media['action_control']
        dynamicBehavior=isbehavior_extraInfo_byUserId_Media['dynamic_behavior']
        monitoredUsersMeetPlayedFilter=isbehavior_extraInfo_byUserId_Media['played_conditional']
        monitoredUsersAction=isbehavior_extraInfo_byUserId_Media['user_conditional']
        ...
        if (actionControl):
            ...
            for itemId in isbehavior_extraInfo_Tracker:
                ...
                for userId in isbehavior_extraInfo_byUserId_Media['userId_list']:
                    ...
                    isMeetingAction_dict=build_behaviorPattern2(isMeetingAction_dict,behavior_pattern_dict,isbehavior_extraInfo_byUserId_Media)
                    isMeetingPlayedFilter_dict=build_behaviorPattern2(isMeetingPlayedFilter_dict,behavior_pattern_dict,isbehavior_extraInfo_byUserId_Media)
                    isMeetingCreatedPlayedFilter_dict=build_behaviorPattern2(isMeetingCreatedPlayedFilter_dict,behavior_pattern_dict,isbehavior_extraInfo_byUserId_Media)

                #all - Every monitored user must have the media item blacklisted/whitelisted/blacktagged/whitetagged/favortied
...
def behavior_playedPatternCleanup2(behavior_str,postproc_dict,the_dict):
    isbehavior_extraInfo_byUserId_Media=postproc_dict['is' + behavior_str + '_extraInfo_byUserId_Media']
    isbehavior_extraInfo_Tracker=isbehavior_extraInfo_byUserId_Media['is' + behavior_str + '_extraInfo_Tracker']
    media_data=postproc_dict['media_data']

    if (('user_conditional' in isbehavior_extraInfo_byUserId_Media) and ('played_conditional' in isbehavior_extraInfo_byUserId_Media) and ('action_type' in isbehavior_extraInfo_byUserId_Media)):

        #assign after verifying isbehavior_extraInfo_byUserId_Media is populated (aka not empty)
        ActionType=isbehavior_extraInfo_byUserId_Media['action_type']
                    ...
                    if (not('IsMeetingAction' in isbehavior_extraInfo_byUserId_Media[userInfo['user_id']][itemId])):
                        ...
                        if ((ActionType == 'blacklisted') or (ActionType == 'whitelisted')):
                            ...
                            if (postproc_dict['media_type_lower'] == 'movie'):
                                isActioned=get_isMOVIE_Tagged(the_dict,media_data[itemId],userInfo,media_tags)
                            elif (postproc_dict['media_type_lower'] == 'episode'):
                                isActioned=get_isEPISODE_Tagged(the_dict,media_data[itemId],userInfo,media_tags)
                            elif (postproc_dict['media_type_lower'] == 'audio'):
                                isActioned=get_isAUDIO_Tagged(the_dict,media_data[itemId],userInfo,media_tags)
                            elif (postproc_dict['media_type_lower'] == 'audiobook'):
                                isActioned=get_isAUDIOBOOK_Tagged(the_dict,media_data[itemId],userInfo,media_tags)
                            else:
                            ...
                            for advFavItem in postproc_dict['advFav_media']:
                                if ((postproc_dict['advFav_media'][advFavItem]) and (isbehavior_extraInfo_byUserId_Media['action_control'])):
                                    if (postproc_dict['media_type_lower'] == 'movie'):
...
def postProcessing2(the_dict,media_dict):
    ...
    '''
    postproc_dict['isblacklisted_extraInfo_byUserId_Media'].update(postproc_dict['blacklisted_behavior_media'])
    postproc_dict['iswhitelisted_extraInfo_byUserId_Media'].update(postproc_dict['whiteisted_behavior_media'])
    postproc_dict['isblacktagged_extraInfo_byUserId_Media'].update(postproc_dict['blacktagged_behavior_media'])
    postproc_dict['iswhitetagged_extraInfo_byUserId_Media'].update(postproc_dict['whitetagged_behavior_media'])
    postproc_dict['isfavorited_extraInfo_byUserId_Media'].update(postproc_dict['favorited_behavior_media'])
    '''

    postproc_dict['isblacklisted_extraInfo_byUserId_Media']=postproc_dict['blacklisted_behavior_media']
    postproc_dict['iswhitelisted_extraInfo_byUserId_Media']=postproc_dict['whitelisted_behavior_media']
    postproc_dict['isblacktagged_extraInfo_byUserId_Media']=postproc_dict['blacktagged_behavior_media']
    postproc_dict['iswhitetagged_extraInfo_byUserId_Media']=postproc_dict['whitetagged_behavior_media']
    postproc_dict['isfavorited_extraInfo_byUserId_Media']=postproc_dict['favorited_behavior_media']

    postproc_dict['isblacklisted_extraInfo_byUserId_Media']['action_type']='blacklisted'
    postproc_dict['iswhitelisted_extraInfo_byUserId_Media']['action_type']='whitelisted'
    postproc_dict['isblacktagged_extraInfo_byUserId_Media']['action_type']='blacktagged'
    postproc_dict['iswhitetagged_extraInfo_byUserId_Media']['action_type']='whitetagged'
    postproc_dict['isfavorited_extraInfo_byUserId_Media']['action_type']='favorited'
    ...
    if (not (media_dict['media_data'] == {})):

        #check media is enabled before post-processing
        if ((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)):

            print_post_processing_started2(the_dict,postproc_dict)

            #for user_key in media_dict:
            for user_info in the_dict['enabled_users']:

                postproc_dict['isblacklisted_extraInfo_byUserId_Media'][user_info['user_id']]={}
                postproc_dict['iswhitelisted_extraInfo_byUserId_Media'][user_info['user_id']]={}
                postproc_dict['isblacktagged_extraInfo_byUserId_Media'][user_info['user_id']]={}
                postproc_dict['iswhitetagged_extraInfo_byUserId_Media'][user_info['user_id']]={}
                postproc_dict['isfavorited_extraInfo_byUserId_Media'][user_info['user_id']]={}

                postproc_dict['mediaCounts_byUserId'][user_info['user_id']]={}

                #if (not ((user_info['user_id'] == 'media_type') or (user_info['user_id'] == 'media_data') or (user_info['user_id'] == 'deleteItemsIdTracker_createdMedia') or
                        #(user_info['user_id'] == 'isfavorited_extraInfo_Tracker') or (user_info['user_id'] == 'iswhitetagged_extraInfo_Tracker') or (user_info['user_id'] == 'isblacktagged_extraInfo_Tracker') or
                        #(user_info['user_id'] == 'iswhitelisted_extraInfo_Tracker') or (user_info['user_id'] == 'isblacklisted_extraInfo_Tracker'))):

                postproc_dict['isblacklisted_extraInfo_byUserId_Media'][user_info['user_id']].update(media_dict[user_info['user_id']]['isblacklisted_extraInfo_byUserId_Media'])
                postproc_dict['iswhitelisted_extraInfo_byUserId_Media'][user_info['user_id']].update(media_dict[user_info['user_id']]['iswhitelisted_extraInfo_byUserId_Media'])
                postproc_dict['isblacktagged_extraInfo_byUserId_Media'][user_info['user_id']].update(media_dict[user_info['user_id']]['isblacktagged_extraInfo_byUserId_Media'])
                postproc_dict['iswhitetagged_extraInfo_byUserId_Media'][user_info['user_id']].update(media_dict[user_info['user_id']]['iswhitetagged_extraInfo_byUserId_Media'])
                postproc_dict['isfavorited_extraInfo_byUserId_Media'][user_info['user_id']].update(media_dict[user_info['user_id']]['isfavorited_extraInfo_byUserId_Media'])

                postproc_dict['mediaCounts_byUserId'][user_info['user_id']].update(media_dict[user_info['user_id']]['mediaCounts_byUserId'])

                '''
                postproc_dict['isblacklisted_extraInfo_byUserId_Media'][user_info['user_id']]=media_dict[user_info['user_id']]['isblacklisted_extraInfo_byUserId_Media']
                postproc_dict['iswhitelisted_extraInfo_byUserId_Media'][user_info['user_id']]=media_dict[user_info['user_id']]['iswhitelisted_extraInfo_byUserId_Media']
                postproc_dict['isblacktagged_extraInfo_byUserId_Media'][user_info['user_id']]=media_dict[user_info['user_id']]['isblacktagged_extraInfo_byUserId_Media']
                postproc_dict['iswhitetagged_extraInfo_byUserId_Media'][user_info['user_id']]=media_dict[user_info['user_id']]['iswhitetagged_extraInfo_byUserId_Media']
                postproc_dict['isfavorited_extraInfo_byUserId_Media'][user_info['user_id']]=media_dict[user_info['user_id']]['isfavorited_extraInfo_byUserId_Media']

                postproc_dict['mediaCounts_byUserId'][user_info['user_id']]=media_dict[user_info['user_id']]['mediaCounts_byUserId']
                '''
            ...
            if (((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)) and (postproc_dict['blacklisted_behavior_media']['action_control'] >= 0)):
                print_post_processing_verbal_progress2(the_dict,postproc_dict,'blacklisted')
                postproc_dict=behavior_playedPatternCleanup2('blacklisted',postproc_dict,the_dict)
                postproc_dict=addItem_removeItem_fromDeleteList_usingBehavioralPatterns2('blacklisted',postproc_dict)
            ...
            if (the_dict['DEBUG']):
                ...
                if ((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)):
                    ...
                    postproc_dict['isblacklisted_extraInfo_byUserId_Media']=convert_timeToString2(postproc_dict['isblacklisted_extraInfo_byUserId_Media'])
            ...
            if (((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)) and (postproc_dict['whitelisted_behavior_media']['action_control'] >= 0)):
                print_post_processing_verbal_progress2(the_dict,postproc_dict,'WHITELISTED')
                postproc_dict=behavior_playedPatternCleanup2('whitelisted',postproc_dict,the_dict)
                postproc_dict=addItem_removeItem_fromDeleteList_usingBehavioralPatterns2('whitelisted',postproc_dict)
            ...
            if (the_dict['DEBUG']):
                ...
                if ((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)):
                    ...
                    postproc_dict['iswhitelisted_extraInfo_byUserId_Media']=convert_timeToString2(postproc_dict['iswhitelisted_extraInfo_byUserId_Media'])
            ...
            if (((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)) and (postproc_dict['blacktagged_behavior_media']['action_control'] >= 0)):
                print_post_processing_verbal_progress2(the_dict,postproc_dict,'BLACKTAGGED')
                postproc_dict=behavior_playedPatternCleanup2('blacktagged',postproc_dict,the_dict)
                postproc_dict=addItem_removeItem_fromDeleteList_usingBehavioralPatterns2('blacktagged',postproc_dict)
            ...
            if (the_dict['DEBUG']):
                ...
                if ((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)):
                    ...
                    postproc_dict['isblacktagged_extraInfo_byUserId_Media']=convert_timeToString2(postproc_dict['isblacktagged_extraInfo_byUserId_Media'])
            ...
            if (((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)) and (postproc_dict['whitetagged_behavior_media']['action_control'] >= 0)):
                print_post_processing_verbal_progress2(the_dict,postproc_dict,'WHITETAGGED')
                postproc_dict=behavior_playedPatternCleanup2('whitetagged',postproc_dict,the_dict)
                postproc_dict=addItem_removeItem_fromDeleteList_usingBehavioralPatterns2('whitetagged',postproc_dict)
            ...
            if (the_dict['DEBUG']):
                ...
                if ((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)):
                    ...
                    postproc_dict['iswhitetagged_extraInfo_byUserId_Media']=convert_timeToString2(postproc_dict['iswhitetagged_extraInfo_byUserId_Media'])
            ...
            if (((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)) and (postproc_dict['favorited_behavior_media']['action_control'] >= 0)):
                print_post_processing_verbal_progress2(the_dict,postproc_dict,'FAVORITED')
                postproc_dict=behavior_playedPatternCleanup2('favorited',postproc_dict,the_dict)
                postproc_dict=addItem_removeItem_fromDeleteList_usingBehavioralPatterns2('favorited',postproc_dict)
            ...
            if (the_dict['DEBUG']):
                ...
                if ((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)):
                    ...
                    postproc_dict['isfavorited_extraInfo_byUserId_Media']=convert_timeToString2(postproc_dict['isfavorited_extraInfo_byUserId_Media'])
            ...
            if (postproc_dict['media_type_lower'] == 'episode'):
                ...
                if (((postproc_dict['media_played_days'] >= 0) or (postproc_dict['media_created_days'] >= 0)) and ((postproc_dict['minimum_number_episodes'] >= 1) or (postproc_dict['minimum_number_played_episodes'] >= 1))):
                    print_post_processing_verbal_progress_min_episode2(the_dict,postproc_dict)
            ...
            print_post_processing_completed2(the_dict,postproc_dict)
        ...
        if (not (postproc_dict['media_created_behavioral_control'])):
            for itemId in postproc_dict['deleteItemsIdTracker_createdMedia']:
                if (not (itemId in postproc_dict['deleteItems_Tracker'])):
                    if (itemId in postproc_dict['media_data']):
                        postproc_dict['deleteItems_Tracker'].append(itemId)
...
def start_postProcessing2(the_dict,media_dict,deleteItems_dict):
    if (not (the_dict['all_media_disabled'])):
        #perform post processing for each media type
        deleteItems_media=postProcessing2(the_dict,media_dict)
...
def init_postProcessing2(media_type,the_dict):

    media_dict=the_dict[media_type + '_dict']
    ...
    if (not (the_dict['DEBUG'])):
        ...
        mpp_media_post_process=multiprocessing.Process(target=start_postProcessing2,args=(the_dict,media_dict,deleteItems_dict))
        #mpp_movie_post_process=multiprocessing.Process(target=start_postProcessing2,args=(the_dict,movie_dict,deleteItems_dict))
        #mpp_episodePostProcess=multiprocessing.Process(target=start_postProcessing2,args=(the_dict,episode_dict,deleteItems_dict))
        #mpp_audioPostProcess=multiprocessing.Process(target=start_postProcessing2,args=(the_dict,audio_dict,deleteItems_dict))
        #if (isJellyfinServer(the_dict['admin_settings']['server']['brand'])):
            #mpp_audiobookPostProcess=multiprocessing.Process(target=start_postProcessing2,args=(the_dict,audiobook_dict,deleteItems_dict))

        #start all multi processes
        #order intentially: Audio, Episodes, Movies, Audiobooks
        #if (isJellyfinServer(the_dict['admin_settings']['server']['brand'])):
        mpp_media_post_process.start()
        ...
        deleteItems_dict=start_postProcessing2(the_dict,media_dict,deleteItems_dict)
        #deleteItems_dict=start_postProcessing2(the_dict,movie_dict,deleteItems_dict)
        #deleteItems_dict=start_postProcessing2(the_dict,episode_dict,deleteItems_dict)
        #deleteItems_dict=start_postProcessing2(the_dict,audio_dict,deleteItems_dict)
        #if (isJellyfinServer(the_dict['admin_settings']['server']['brand'])):
            #deleteItems_dict=start_postProcessing2(the_dict,audiobook_dict,deleteItems_dict)
        #else:
            #deleteItems_dict['audiobook']=[]
