...
def get_ADDITIONAL_itemInfo(user_key,itemId,lookupTopic,the_dict):
    ...
    itemInfo=requestURL(url, the_dict['DEBUG'], lookupTopic, the_dict['api_query_attempts'], the_dict)
...
def get_SERIES_itemInfo(episode,user_key,the_dict):
...
### Series ########################################################################################
    ...
    if ('SeriesId' in episode):
        series_item_info = get_ADDITIONAL_itemInfo(user_key,episode['SeriesId'],'series_info',the_dict)

    elif ('SeasonId' in episode):
        season_item_info = get_ADDITIONAL_itemInfo(user_key,episode['SeasonId'],'season_info',the_dict)
        ...
        season_item_info = get_ADDITIONAL_itemInfo(user_key,episode['ParentId'],'season_info',the_dict)
